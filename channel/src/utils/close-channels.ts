...
class CloseClient {
  ...
  async getChannels(walletAddress: Address): Promise<RPCChannelUpdateWithWallet[]> {
    const msg = createGetChannelsMessageV2(walletAddress)
    const res = await this.send(msg) as { res: [number, string, { channels: RPCChannelUpdateWithWallet[] }] }
    return res.res[2].channels ?? []
  }

  async closeChannelRPC(wallet: WalletData, channelId: Hex): Promise<RPCChannelOperation> {
    const msg = await createCloseChannelMessage(wallet.signer, channelId, wallet.address as Address)
    const res = await this.send(msg) as { res: [number, string, RPCChannelOperation] }
    return res.res[2]
  }
...
async function closeWalletChannels(wallet: WalletData, config: ConfigResponse): Promise<void> {
  ...
  try {
    ...
    for (const ch of openChannels) {
      console.log(`    Closing channel ${ch.channelId}...`)

      // 1. Request close from ClearNode
      const closeRes = await client.closeChannelRPC(wallet, ch.channelId)

      // 2. Build FinalState from response
      const finalState: FinalState = {
        channelId: closeRes.channelId,
        intent: StateIntent.FINALIZE,
        version: BigInt(closeRes.state.version),
        data: closeRes.state.stateData,
        allocations: closeRes.state.allocations.map((a) => ({
          destination: a.destination,
          token: a.token,
          amount: BigInt(a.amount),
        })),
        serverSignature: closeRes.serverSignature,
      }

      // 3. Close on-chain
      const params: CloseChannelParams = { finalState }
    ...
    // 4. Withdraw funds from custody
    for (const [token, amount] of tokenAmounts) {
      console.log(`    Withdrawing ${amount} of ${token}...`)
      const txHash = await nitroliteClient.withdrawal(token, amount)
    ...
    for (const token of [ETH_ADDRESS, USDC_ADDRESS]) {
      ...
      const decimals = token === ETH_ADDRESS ? 18 : 6
      const formatted = token === ETH_ADDRESS ? formatEther(balance) : formatUnits(balance, decimals)
